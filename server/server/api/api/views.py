...
def data_aggregation(data):
    ...
    aggregated_data['avg_military_service'] = Counter(military_service)

    is_active = [d['is_active'] for d in data]
    aggregated_data['avg_is_active'] = Counter(is_active)

    nominating_party = [d['nominating_party'] for d in data]
    aggregated_data['avg_nominating_party'] = Counter(nominating_party)

    law_school = [d['law_school'] for d in data]
    aggregated_data['avg_law_school'] = Counter(law_school)

    state_of_birth = [d['state_of_birth'] for d in data]
    aggregated_data['avg_state_of_birth'] = Counter(state_of_birth)
